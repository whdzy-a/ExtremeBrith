...
{
    public class BigWhiskers : Template
    {
        ...
        public BigWhiskers(LizardGraphics lGraphics, int startSprite)
            : base(lGraphics, startSprite)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                whiskerProps[i, 0] = Custom.ClampedRandomVariation(0.5f, 0.4f, 0.5f) * 55f;
                whiskerProps[i, 1] = Mathf.Lerp(0.1f, -0.2f, Random.value);
                whiskerProps[i, 2] = Mathf.Lerp(40, 40, Mathf.Pow(Random.value, 1.5f)) / whiskerProps[i, 0];
                whiskerProps[i, 3] = Random.value;
                whiskerProps[i, 4] = Mathf.Lerp(1.4f, 2.2f, Mathf.Pow(Random.value, 1.6f));
                if (i <= 0)
                {
                    continue;
                }
            ...
            for (int i = 0; i < amount; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        ...
                        {
                            (sLeaser.sprites[startSprite + i * 2 + j] as TriangleMesh).verticeColors[l] = Color.Lerp(lGraphics.HeadColor(timeStacker), new Color(1f, 1f, 1f), (k - 1) / 2f * Mathf.Lerp(whiskerLightUp[i, j, 1], whiskerLightUp[i, j, 0], timeStacker));
                        }
