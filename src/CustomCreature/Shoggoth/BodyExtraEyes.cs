...
{
    internal class BodyExtraEyes : Template
    {
        ...
        public BodyExtraEyes(LizardGraphics lGraphics, int startSprite)
        ...
        public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (i == 0)
                {
                    vector = (vector - DirVec(Vector2.Lerp(lGraphics.drawPositions[0, 1], lGraphics.drawPositions[0, 0], timeStacker), Vector2.Lerp(lGraphics.head.lastPos, lGraphics.head.pos, timeStacker))).normalized;
                }

                Vector2 vector2 = PerpendicularVector(vector);
                float num2 = 50f * Mathf.Lerp(a, b, i == 0 ? 0.25f : 0.5f);
                for (int j = 0; j < 2; j++)
                {
                    Vector2 vector3 = Custom.DegToVec(num2 + (j == 0f ? -40f : 40f));
                    Vector2 p = pos + vector2 * lizardSpineData.rad * vector3.x;
                    Vector2 vector4 = vector;
                    vector4 = i != 0 ? (vector4 + 2f * DirVec(p, Vector2.Lerp(lGraphics.tail[0].lastPos, lGraphics.tail[0].pos, timeStacker)) * Mathf.Abs(vector3.y)).normalized : (vector4 - 2f * DirVec(p, Vector2.Lerp(lGraphics.head.lastPos, lGraphics.head.pos, timeStacker)) * Mathf.Abs(vector3.y)).normalized;
        ...
        // 保留原版的工具方法
        public static Vector2 DirVec(Vector2 p1, Vector2 p2) => p1 == p2 ? new Vector2(0f, 1f) : (p2 - p1).normalized;
        public static Vector2 PerpendicularVector(Vector2 v) => new Vector2(-v.y, v.x).normalized;
    }
}